---
title: "Usage API"
description: "Public API to ingest usage events for billing and metering."
sidebarTitle: "Usage API"
---

# ➕ Add Usage

This API allows clients to report usage events to the billing system for processing and aggregation. These events are grouped and priced based on contract configuration and usage drivers.

---

## 📥 Endpoint

**POST** `https://www.data.valyx.com/usage`

```
POST /usage HTTP/1.1
Host: data.valyx.com
Authorization: Bearer sk_live_1234567890abcdef
Content-Type: application/json
```

---

### 📤  `curl` Request

```bash
curl -X POST \
  'http://localhost:8181/usage' \
  -H 'accept: application/json' \
  -H 'Authorization: Bearer <your_api_key>' \
  -H 'Content-Type: application/json' \
  -d '{
    "buyerId": "string",
    "contractId": "string",
    "eventId": "string",
	"eventTimeStamp": int
    "tags": {
      "additionalProp1": "string",
      "additionalProp2": "string",
      "additionalProp3": "string"
    },
    "metrics": [
      {
        "usage_driver_id": "string",
        "quantity": int,
        "count": int
      },
      {
        "usage_driver_id": "string",
        "quantity": int,
        "count": int
      }
    ],
    "usageReferenceId": "string"
  }'
```

## Authorization

<AccordionGroup>
  <Accordion title="Authorization" description="API key is passed as a Bearer token in the header" icon="key">
    Include your API key in the request header using the **Bearer** scheme.

    ```http
    Authorization: Bearer <your_api_key>
    ```

    This is required for authenticating the request and identifying the calling company.
  </Accordion>
</AccordionGroup>

---

## Body

<AccordionGroup>
  <Accordion title="buyerId" description="Unique identifier for the buyer" icon="tag">
    `buyerId` `string` `required`\
    Example: `BUY12345`
  </Accordion>
  <Accordion title="contractId" description="Contract ID under which usage is recorded" icon="file-signature">
    `contractId` `string` `optional`\
    If not provided, it will be resolved using `buyerId` and `X-Company-ID`.\
    Example: `CON45678`
  </Accordion>
  <Accordion title="eventId" description="Globally unique event ID" icon="fingerprint">
    `eventId` `string` `optional`\
    Ensures idempotency. If the same ID is reused, the latest event will overwrite previous ones.\
    Example: `EVT_API_REQUEST`
  </Accordion>
  <Accordion title="eventTimestamp" description="Unix timestamp (millis) for when the event occurred" icon="clock">
    `eventTimestamp` `int64` `optional`\
    Example: `1721502930`
  </Accordion>
  <Accordion title="tags" description="Custom key-value tags to drive pricing logic" icon="labels">
    `tags` `object` `required`\
    Example:

    ```json
    {
      "region": "us-east-1",
      "tier": "pro",
      "feature": "api"
    }
    ```
  </Accordion>
  <Accordion title="metrics" description="Defines a list of usage drivers along with how much and how many times each was consumed" icon="ruler">
    Each item in the `usageDrivers` array represents a specific usage driver and includes:

    - `usage_driver_id` `string` `required`\
      Unique identifier of the usage driver.\
      Example: `UDR00123`
    - `quantity` `integer` `required`\
      The amount of the resource that was consumed.\
      Example: `10`
    - `count` `integer` `required`\
      The number of times the usage driver was triggered/used.\
      Example: `3`
  </Accordion>
  <Accordion title="usageReferenceId" description="Used to group related usage events for aggregation" icon="link">
    `usageReferenceId` `string` `required`\
    Example: `REF7890`
  </Accordion>
</AccordionGroup>

---

## Response

<Tabs>
  <Tab title="201">
    ```
    {
      "status": "success",
      "message": "Usage data recorded successfully.",
    }
    
    ```
  </Tab>
  <Tab title="500">
    ```json
    {
      "details": "fetch customer details: no buyer ID 'string' exists : Error 1146 (42S02): couldn't find customer with id buyer_1234",
      "error": "Failed to process usage data"
    }
    ```
  </Tab>
  <Tab title="400">
    ```json
    {
      "error": "Invalid request body",
      "details": "json: cannot unmarshal string into struct field CreateUsageEvent.quantity of type float64"
    }
    
    ```
  </Tab>
  <Tab title="422">
    ```json
    {
      "error": "Unprocessable Entity",
      "details": "could not fetch contractID: contract not found  for company_1234 and customer_9876"
    }
    
    ```
  </Tab>
  <Tab title="422">
    ```json
    {
      "error": "Unprocessable Entity",
      "details": "contract ID validation failed: failed to fetch contract details: "
    }
    
    ```
  </Tab>
  <Tab title="422">
    ```json
    {
      "error": "Unprocessable Entity",
      "details": "event timestamp 17834059734 is outside contract period [01-01-2025 to 31-01-2025]"
    }
    
    ```
  </Tab>
  <Tab title="422">
    ```json
    {
      "error": "Unprocessable Entity",
      "details": "usage driver validation failed: failed to fetch usage driver status"
    }
    
    ```
  </Tab>
  <Tab title="422">
    ```json
    {
      "error": "Unprocessable Entity",
      "details": "  failed to query billing approvals"
    }
    
    ```
  </Tab>
</Tabs>

### 📘 Notes

- The API is idempotent based on eventId.
- Use usageReferenceId for batch-level billing or to logically group usage activity.
- Invalid or unmatched tags will result in event rejection due to missing rate card linkage.
- Always ensure that eventTimestamp falls within the contract validity period.